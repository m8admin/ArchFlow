                      ...
                      ...p.milestones.flatMap(m => {
                        ...
                        return [
                          <tr key={m.id} className="tk">
                            <td className="ind" style={{ fontWeight: 600 }}>↪ {m.phase && <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--pu)', background: 'var(--pu-bg)', padding: '2px 8px', borderRadius: 12, marginRight: 6 }}>{m.phase}</span>}<span className="pnc" onClick={() => onEditTask(m.id)}>{m.name}</span>{m.modeller_hours ? <span style={{ fontSize: 11, color: 'var(--tx3)', fontWeight: 400, marginLeft: 6 }}>{m.modeller_hours}h</span> : null}{isAdmin && hoursByTask && (m.modeller_hours || hoursByTask[m.id]) ? (() => { const actual = hoursByTask[m.id] || 0; const planned = m.modeller_hours || 0; const over = planned > 0 && actual > planned; return <span style={{ fontSize: 10, fontWeight: 600, marginLeft: 6, padding: '1px 6px', borderRadius: 10, background: over ? 'var(--rd-bg)' : 'var(--gn-bg)', color: over ? 'var(--rd)' : 'var(--gn)' }}>{actual.toFixed(1)}/{planned || '?'}h</span> })() : null}</td>
